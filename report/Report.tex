...
Our \texttt{Query} class applies filter predicates one column at a time via a chainable API (\texttt{.filter("year","==",y).filter("town","in",towns)...min("price\_per\_sqm")}). Only the relevant column arrays are iterated; surviving row indices are progressively narrowed. For string columns, values are resolved to dictionary codes before comparison, making filters integer set-membership tests.
...
Vectorized + Optimized      & \multicolumn{2}{c}{(64 chunks, query sum 0.050\,s)} & 7.321 & 0.18\,MiB \\
\bottomrule
\end{tabular}
...
\hline
 &  & Discussion, testing code, report writing & 33\% \\
\hline
 &  & Discussion, testing code, report writing & 33\% \\
\hline
\end{tabular}
...
\includegraphics[height=1.8cm]{cong.jpg} & \\[-0.2cm]
\rule{6cm}{0.4pt} & \rule{6cm}{0.4pt} \\
Pham Ba Cong (Member 1) & Name and Signature of Member 2 \\[1.8cm]
\rule{6cm}{0.4pt} & \rule{6cm}{0.4pt} \\
